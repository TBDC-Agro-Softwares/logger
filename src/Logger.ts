...
export default abstract class Logger {
  ...
  static debug(title: string, message: string): void {
    console.debug(`[${colors.blue(title)}] ${getCurrentTimeFormatted()} ${message}`);
  }

  static info(message: string): void {
    const config = getConfig();
    if (config.logLevel !== 'debug') return;
    console.info(`[${colors.cyan('INFO')}] ${getCurrentTimeFormatted()} ${message}`);
  }

  static warn(message: string): void {
    const config = getConfig();
    if (!['debug', 'warning'].includes(config.logLevel)) return;

    console.warn(`[${colors.yellow('WARN')}] ${getCurrentTimeFormatted()} ${message}`);
  }

  static error(message: string | undefined): void {
    const error = message ?? 'No stack trace.';
    console.error(`${colors.bgRed(' ERROR ')} ${getCurrentTimeFormatted()} ${error}`);
  }
  ...
  static criticalError(origin: string, message: string | undefined): void {
    ...
    console.error(`${colors.bgRed(colors.white(originText))} ${colors.red(messageText)}`);
  }
  ...
  static success(title: string, message: string): void {
    console.log(`[${colors.green(title)}] ${getCurrentTimeFormatted()} ${message}`);
  }
  ...
  static httpDebugStart(method: string, route: string): void {
    ...
    if (config.logLevel === 'debug') {
      console.debug(colors.gray(`[${method}] ${getCurrentTimeFormatted()} ${route}`));
    }
  }
  ...
  static httpDebugEnd(method: string, route: string, status: number, responseTime: number): void {
    ...
    const config = getConfig();
    if (config.logLevel === 'debug' || responseTime > 5000) {
      console.debug(`${httpMethod} ${getCurrentTimeFormatted()} ${route} ${statusCode} ${timeInMilliseconds}`);
    }
  }
}
